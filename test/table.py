...
def table_results(model_name, parameter_name, results_dict, format_title=True):
    if format_title:
        title = f"{model_name} Test Parameter: {parameter_name}"
    else:
        title = model_name
    ...
    console.print(table)
    console.save_svg(f"fig/table_{model_name}_{parameter_name}.svg")